...
public class Decode
{
    ...
    public static void decode (MDeque<Integer> list , String instructions) throws NoSuchElementException
    {
        for(int i = 0;i<instructions.length();i++)
        {
            ...
            {
                Iterator itr = list.reverseIterator();
                if(list.size() >1)
                {
                    int original = list.size();
                    for (int first = 0; first < original; first++)
                    {
                        list.pushBack((Integer) itr.next());
                    }
                    for (int remove = 0; remove < original; remove++)
                    {
                        list.popFront();
                    }
                }
    ...
    public static boolean isValid ( String instructions )
    {
        ...
        {
            String a = instructions.substring(0,1);
            if((a.equals("F")||a.equals("B"))||a.equals("R"))
            {
